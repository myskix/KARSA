...
export const BottomNav = (activeTab = 'home') => {
  const tabs = [
    ...
    { id: 'jejak', label: 'Life Simulation', icon: 'M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
    { id: 'peta', label: 'Peta Langkah', icon: 'M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7' }
  ];

  const renderTab = (tab) => {
    const isActive = activeTab === tab.id;
    const colorClass = isActive ? 'text-karsa-primary' : 'text-slate-400 hover:text-slate-600';
    const activeDot = isActive ? `<div class="w-1 h-1 bg-karsa-primary rounded-full mt-1"></div>` : `<div class="w-1 h-1 mt-1"></div>`;
    ...
    return `
      <a href="#${tab.id}" class="flex flex-col items-center justify-center w-full py-2 ${colorClass} transition-colors active:scale-95 touch-manipulation">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mb-1 ${isActive ? 'fill-current opacity-20 absolute' : 'hidden'}" viewBox="0 0 24 24"></svg>
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 z-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
